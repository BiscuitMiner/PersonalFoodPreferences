...
PFP 會在讀取 `FoodCategoryExtension` 時檢查明確壞資料。空白 `category` 本身不是錯誤；它表示不指定固定 primary，讓食材分析、foodType 或 `GenericFood` 後續接手。

- `Invalid fallbackCategory`
  - 原因：`fallbackCategory` 有填值，但不是 PFP 13 類標準分類之一。
  - 修正：改成合法標準分類；若沒有需要 fallback，可移除此欄位或留空。

- `Invalid tag`
  - 原因：`tags` 中至少有一項不是 PFP 13 類標準分類。
  - 修正：刪除無效 tag，或改成合法標準分類。自訂語意分類應放在 `category`，不要放在 `tags`。

- `has multiple FoodCategoryExtension entries`
  - 原因：同一個 `ThingDef` 被多個 patch 或基礎 Def 重複加入 `PersonalFoodPreferences.FoodCategoryExtension`。
  - 修正：相容 patch 的 XPath 應使用 `not(li[@Class="PersonalFoodPreferences.FoodCategoryExtension"])`，避免重複加入。

如果看到 RimWorld XML 類型解析錯誤，而不是 PFP extension warning，常見原因是你的基礎 Def 或無條件 patch 在 PFP 未啟用時仍引用 `PersonalFoodPreferences.FoodCategoryExtension`。這種情況應改用 `MayRequire` 或 `PatchOperationFindMod` 做可選相容。
...
1. 啟用 PFP 和你的 MOD。
2. 啟動遊戲，確認沒有 XML 類型解析錯誤、無效 fallback、無效 tag 或重複 extension warning。
